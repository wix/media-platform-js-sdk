...
describe('widget instances', () => {
  ...
  const widgetInstanceManager = new WidgetInstanceManager(configuration, httpClient);
  ...
  it('should return widget instance', () => {
    apiServer
      ...
      .replyWithFile(200, path.join(mockFolderPath, 'widget-instance-auth.mock.json'));

    apiServer
      .get(`/_api/widgetInstances/${widgetInstanceId}`)
      .replyWithFile(200, path.join(mockFolderPath, 'widget-instance.mock.json'));
