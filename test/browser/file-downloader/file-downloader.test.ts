  ...
  it('should download a file with `downloadUrlRequest` params', async () => {
    ...
    fauxJax.on('request', (request) => {
      if (
        ...
        expect(request.requestURL).to.be.eql(
          'https://www.domain.com/_api/download/secure_url?path=%2Fimg2.jpg&saveAs=save-as-name&expirationRedirectUrl=redirect-link&expiry=357',
        );
      }
    });
    ...
    const downloadUrl = await fileDownloader.getDownloadUrl(
      '/img2.jpg',
      downloadUrlRequest,
    );
