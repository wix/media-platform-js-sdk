...
export default function(app) {
  app.get('/media-platform/file/upload/configuration', function(req, res) {
    const fileManager = mediaPlatform.fileManager;

    fileManager.getUploadConfiguration().then(
      uploadCredentials => {
  ...
  app.get('/media-platform/files', function(req, res) {
    ...
    fileManager.listFiles('/demo', listFilesRequest).then(
      listFilesResponse => {
  ...
  app.get('/media-platform/file/:id/metadata', function(req, res) {
    ...
    fileManager.getFileMetadataById(req.params.id).then(
      response => {
        res.send(response);
      },
      error => {
  ...
  app.get('/media-platform/file/download/url', function(req, res) {
    const url = mediaPlatform.getSignedUrl(req.query.path);
  ...
  app.get('/media-platform/file/import', function(req, res) {
    ...
    const importFileRequest = new ImportFileRequest({
      sourceUrl:
        'https://static.wixstatic.com/media/f31d7d0cfc554aacb1d737757c8d3f1b.jpg',
      destination: {
    ...
    fileManager.importFile(importFileRequest).then(
      response => {
        res.send(response);
      },
      error => {
